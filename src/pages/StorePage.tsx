...
const StorePage: FC = () => {
  ...
  const itemCounts = useMemo(() => {
    return {
      ...products.reduce<Record<keyof Pick<Product, 'brand' | 'category'>, Record<string, number>>>(
        (initial, item) => {
  ...
  return (
    <div className='grid grid-cols-1 grid-rows-1 xl:grid-cols-[1fr,4fr] xl:gap-4 overflow-hidden'>
      <div
        className={clsx(
          'col-span-full row-span-full flex w-[100%] -translate-x-full transition-transform xl:col-auto xl:translate-x-0',
              ...
              <List
                ...
                fn={([key, value]) => (
                  <FilterControl
                    ...
                    info={
                      <>
                        ({itemCounts.category[key] ?? 0} / {value})
                      </>
                    }
                    handle={(checked) =>
                      setSearch((builder) =>
                        checked ? builder.append('category', key) : builder.remove('category', key)
                      )
                    }
                  />
                )}
              ...
              <List
                ...
                fn={([key, value]) => (
                  <FilterControl
                    ...
                    info={
                      <>
                        ({itemCounts.brand[key] ?? 0} / {value})
                      </>
                    }
                    handle={(checked) =>
                      setSearch((builder) =>
                        checked ? builder.append('brand', key) : builder.remove('brand', key)
                      )
                    }
                  />
                )}
      ...
      <div className='col-span-full row-span-full space-y-4 xl:col-auto xl:row-auto'>
        <div className='flex items-center justify-between flex-wrap'>
          <div>
            <button className='xl:hidden' onClick={() => setIsSidebarOpen(true)}>
              <IoFilter />
            </button>
          ...
          <div>
            Found: <span className='font-bold'>{products.length}</span>
          </div>

          <div className='flex items-center gap-x-4'>
            <SearchControl
              value={search.search}
              handle={(value) => setSearch((builder) => builder.set('search', value))}
            />
        ...
        {products.length > 0 ? (
          <div
            className={clsx('grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-3', {
              'grid-cols-[repeat(auto-fit,minmax(200px,1fr))]': search.view?.includes('small'),
            })}
          >
            <List
              items={products}
