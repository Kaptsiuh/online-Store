...
const CartPromoForm: FC<CartPromoFormProps> = (props) => {
  ...
  const newPrice = useMemo(
    () => price * (1 - promos.reduce((sum, { disc }) => sum + disc, 0) / 100),
    [promos, price]
  )
  ...
  return (
    <div className='flex flex-col items-center border px-4 py-2 gap-y-2'>
      <div className={clsx('text-xl', promos.length > 0 && 'line-through')}>
        Total: <span className='font-medium'>{price}</span>$
      </div>
      <div className={clsx('text-xl', promos.length === 0 && 'hidden')}>
        Total: <span className='font-medium'>{newPrice}</span>$
      </div>

      <div>
        Products: <span className='font-medium'>{count}</span>
      </div>
        ...
        <List
          ...
          fn={(item) => (
            <div key={item.id} className='flex items-center justify-center'>
              ...
              <button className='ml-2' onClick={() => onCancelPromo(item)}>
      ...
      {promo && (
        <div className='flex items-center gap-x-2'>
          ...
          {!isApplied && (
            <button onClick={() => onApplyPromo(promo)}>
              <IoAddCircleOutline />
            </button>
          )}
        </div>
      ...
      <button className='button uppercase my-4' onClick={onSubmit}>
        Buy Now
      </button>
    </div>
  )
